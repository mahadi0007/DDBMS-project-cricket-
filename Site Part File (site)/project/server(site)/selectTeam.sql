...
CREATE OR REPLACE PROCEDURE selectTeam(A IN number, B OUT number)
IS
    team number;
...
BEGIN
    ...
    SELECT distinct countryId into country_id  FROM country@server1 WHERE countryName = teamName;
    B := country_id;
    DBMS_OUTPUT.PUT_LINE(teamName||' team players: ');
    i:=0;
    FOR R IN (SELECT playerName FROM playerList@server1 WHERE countryId = country_id ) LOOP
        i:=i+1;
...
CREATE OR REPLACE PROCEDURE OppselectTeam(A IN number, B OUT number)
IS
    oppSelect int;
    oppTeam varchar2(20);
    country_id number;
...
BEGIN
    ...
    LOOP
        ...
        SELECT distinct countryId into country_id  FROM country@server1 WHERE countryName = oppTeam;
        B := country_id;
...
BEGIN
        ...
        FOR R IN (SELECT countryName into allTeamName  FROM country@server1  ) LOOP
                i:=i+1;
...
BEGIN
    ...
    SELECT COUNT(gameId) INTO D FROM scorecard@server1;
    D := D+1;
    insert into scorecard@server1 values(D,B,C);
