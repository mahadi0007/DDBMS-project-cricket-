...
BEGIN
    ...
    FOR R IN (SELECT * FROM player1@server1 WHERE player1ID = game_id) LOOP
        ball := ball+1;
        SELECT playerName into player_name_bat from playerList@server1 WHERE playerId = R.batsmanPlayerId;
        SELECT playerName into player_name_bowl from playerList@server1 WHERE playerId = R.bowlerPlayerId;
