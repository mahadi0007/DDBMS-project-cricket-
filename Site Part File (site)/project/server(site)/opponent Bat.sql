...
BEGIN
    ...
    -- DBMS_OUTPUT.PUT_LINE(i||'game_id = ' || game_id);

    
    SELECT myTeamId INTO player1Team_id FROM scorecard@server1 WHERE gameId=game_id;
    SELECT oppTeamId INTO oppTeam_Id FROM scorecard@server1 WHERE gameId=game_id;
    ...
    insert into opponentScore@server1 values(game_id,oppScore);

    SELECT count(player1ID)  into lastGameID_Player1 FROM player1@server1 WHERE player1ID = game_id ;
